...
function UserCartWrapper({ setOpenCartSheet }) {
  const navigate = useNavigate();
  const {
    items: currentCartItemsArray,
    guestCartId,
    _id: dbCartId,
  } = useSelector((state) => state.shopCart.cartItems);
  const { isAuthenticated } = useSelector((state) => state.auth);

  const totalCartAmount =
    currentCartItemsArray?.reduce(
      // currentCartItemsArray kullanıldı
      (sum, currentItem) =>
        sum +
        (currentItem?.salePrice > 0
          ? currentItem?.salePrice
    ...
    <SheetContent className="sm:max-w-lg flex flex-col max-[500px]:px-0">
      <SheetHeader className="px-6 pt-6 pb-4 border-b">
        ...
        {currentCartItemsArray &&
          currentCartItemsArray.length > 0 && ( // currentCartItemsArray kullanıldı
            <SheetDescription>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-6 space-y-4">
          {currentCartItemsArray && currentCartItemsArray.length > 0 ? ( // currentCartItemsArray kullanıldı
            currentCartItemsArray.map((item) => (
              <UserCartItemsContent
                key={item.productId} // Eğer item içinde benzersiz bir _id varsa onu kullanmak daha iyi
                cartItem={item}
                readOnly={false}
              />
            ))
