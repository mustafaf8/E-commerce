...
const replyToMessage = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Yanıt gönderilemedi.', error: err.message });
  }
};
...
const updateMessageStatus = async (req, res) => {
  try {
    ...
    if (!message) return res.status(404).json({ success: false, message: 'Mesaj bulunamadı.' });
    return res.json({ success: true, message: 'Durum güncellendi.', data: message });
  } catch (err) {
    return res.status(500).json({ success: false, message: 'Durum güncellenemedi.', error: err.message });
  }
};
