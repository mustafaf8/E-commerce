...
exports.initiatePayment = async (req, res) => {
  try {
    ...
    if (!amount || isNaN(parseFloat(amount)) || parseFloat(amount) <= 0) {
      return res
        .status(400)
        .json({ success: false, message: "Geçerli bir tutar giriniz." });
    }
    ...
    const request = {
      ...
      buyer: {
        ...
        email: "gokturklerenerji@gmail.com",
        identityNumber: "11111111111",
        registrationAddress: "Akademi Mah. Oyalı Sk. No:8d Selçuklu/ Konya",
        ip: req.ip || "127.0.0.1",
        city: "Konya",
        country: "Turkey",
        zipCode: "42250",
      ...
      shippingAddress: {
        contactName: "Göktürkler Enerji",
        city: "Konya",
        country: "Turkey",
        address: "Akademi Mah. Oyalı Sk. No:8d Selçuklu/ Konya",
        zipCode: "42250",
      },
      billingAddress: {
        contactName: "Göktürkler Enerji",
    ...
    iyzipay.checkoutFormInitialize.create(request, (err, result) => {
      if (err || result.status === "failure") {
        ...
        return res
          .status(500)
          .json({ success: false, message: "Ödeme başlatılamadı." });
      }
...
exports.handleCallback = async (req, res) => {
  ...
  if (!token) {
    return res.redirect(
      `${clientRedirectBase}?status=error&message=TokenNotFound`
    );
  }
  ...
  iyzipay.checkoutForm.retrieve(
    ...
    async (err, result) => {
      ...
      if (err || result.status === "failure") {
        ...
        return res.redirect(
          `${clientRedirectBase}?status=failed&id=${
            paymentRecordId || ""
          }&message=${result?.errorMessage || "UnknownError"}`
        );
      }
      ...
      if (result.paymentStatus === "SUCCESS") {
        ...
        return res.redirect(
          `${clientRedirectBase}?status=success&id=${paymentRecordId}`
        );
      } else {
        await DirectPayment.findByIdAndUpdate(paymentRecordId, {
          status: "failed",
        ...
        return res.redirect(
          `${clientRedirectBase}?status=failed&id=${paymentRecordId}`
        );
      }
    }
  );
